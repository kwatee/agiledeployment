...
static uint1 buffer[blocksize]; // bytes that didn't fit in last 64 byte chunk
static uint4 count[2];   // 64bit counter for number of bits (lo, hi)
static uint4 state[4];   // digest so far
static uint1 digest[16]; // the result
...
void decode(uint4 output[], uint1 input[], size_t len) {
	unsigned int i, j;
...
// a multiple of 4.
void encode(uint1 output[], uint4 input[], size_t len) {
	size_t i, j;
	for (i = 0, j = 0; j < len; i++, j += 4) {
...
void transform(uint1 block[blocksize]) {
...
void md5_update(unsigned char input[], size_t length) {
