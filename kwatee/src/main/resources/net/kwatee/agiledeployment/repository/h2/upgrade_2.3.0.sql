ALTER TABLE KWDeployment ADD COLUMN has_errors BOOLEAN NOT NULL DEFAULT 0;

ALTER TABLE KWPackage DROP CONSTRAINT kwuc_package_name;
...
ALTER TABLE KWServer DROP CONSTRAINT kwuc_server_name;
CREATE UNIQUE INDEX index_KWServer ON KWServer (name);

CREATE INDEX index_KWServerProperty ON KWServerProperty (server_id);

ALTER TABLE KWEnvironment DROP CONSTRAINT kwuc_environment_name;
...
ALTER TABLE KWUser DROP CONSTRAINT kwuc_user_login;
...
ALTER TABLE KWSystemProperty DROP CONSTRAINT kwuc_systemproperty_name;
